...
2. **Address every CRITICAL Red Team finding in THIS rewrite.** CRITICAL findings MUST be resolved in the current depth's plan — not deferred, not routed to follow-up, not filed for later. If a prior Red Team envelope returned BLOCK with a CRITICAL security finding (e.g., committed secret, injection path, auth gap), the rewrite's affected phase MUST be rewritten to close the finding directly. No "address in follow-up" language for CRITICAL. MAJOR findings should be addressed unless justified. MINOR findings may be noted and deferred.
